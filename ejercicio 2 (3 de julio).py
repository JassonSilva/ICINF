...
def modificar_elemento(lista):
    try:
        indice = int(input("Ingrese el índice del elemento a modificar (comienza desde 1): ")) - 1
        if 0 <= indice < len(lista):
            nuevo_elemento = input("Ingrese el nuevo valor del elemento: ")
            lista[indice] = nuevo_elemento
            print(f"Elemento en el índice {indice + 1} modificado a '{nuevo_elemento}'.")
        else:
...
def eliminar_elemento(lista):
    try:
        indice = int(input("Ingrese el índice del elemento a eliminar (comienza desde 1): ")) - 1
        if 0 <= indice < len(lista):
...
def buscar_elemento(lista):
    ...
    if elemento in lista:
        ...
        print(f"El elemento '{elemento}' se encuentra en el índice {indice + 1}.")
    else:
